...
def listar_periodo_reservas(limit, offset, fecha=None):
    ...
    reservas = execute(f"""
        SELECT r.id, s.price, s.start_time, s.end_time,
...
def get_dashboard_data(fecha, limit=100, offset=0):
    ...
    if total > 0:
        reservas = execute(f"""
            SELECT r.id,
                   s.price, s.start_time, s.end_time, s.map_id,
                   a.name as user_name, a.dni as dni_usuario,
                   m.name as map_name
            FROM Reservations r
            JOIN Accounts a ON r.account_id = a.id
    ...
    dia_data = execute(f"SELECT COALESCE(SUM(s.price), 0) as total FROM Reservations r JOIN Salas s ON r.sala_id = s.id WHERE s.reservation_date = '{fecha}' AND r.canceled = FALSE")
    if dia_data:
        ingresos['dia'] = int(dia_data[0]['total'])

    semana_data = execute(f"SELECT COALESCE(SUM(s.price), 0) as total FROM Reservations r JOIN Salas s ON r.sala_id = s.id WHERE YEARWEEK(s.reservation_date, 1) = YEARWEEK('{fecha}', 1) AND r.canceled = FALSE")
    if semana_data:
        ingresos['semana'] = int(semana_data[0]['total'])

    mes_data = execute(f"SELECT COALESCE(SUM(s.price), 0) as total FROM Reservations r JOIN Salas s ON r.sala_id = s.id WHERE MONTH(s.reservation_date) = MONTH('{fecha}') AND YEAR(s.reservation_date) = YEAR('{fecha}') AND r.canceled = FALSE")
    if mes_data:
        ingresos['mes'] = int(mes_data[0]['total'])

    anio_data = execute(f"SELECT COALESCE(SUM(s.price), 0) as total FROM Reservations r JOIN Salas s ON r.sala_id = s.id WHERE YEAR(s.reservation_date) = YEAR('{fecha}') AND r.canceled = FALSE")
    if anio_data:
